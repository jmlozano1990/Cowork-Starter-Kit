...
## Phase-5 Addendum (2026-07-19) — AC-SEC-S1 hardened after a @qa-found bypass (QA-1)

The Phase-2 AC-SEC-S1 check as first bound (`printf '%s' "$slug" | grep -qE '^[a-z0-9][a-z0-9-]*$'`) was **insufficient** — a genuine BLOCKER surfaced only at the independent @qa gate (Phase 5), not at Phase 2 or in the orchestrator's Phase-4 re-verification. `grep -E` is **line-oriented**: `^`/`$` anchor per line, and `grep -q` succeeds if *any* line matches. A two-line slug (`decision-log` + newline + `x -->evil<!--`) therefore PASSED the gate, and once embedded in the AC-P1-1 marker the second line broke out of the HTML comment and injected `evil` as visible body text into the auto-loaded `CLAUDE.md` — the exact failure class S1 exists to close, via an embedded-newline payload that neither the Phase-2 review nor the Phase-4 re-verify tested. **Fix (shipped):** whole-string match `[[ "$slug" =~ ^[a-z0-9][a-z0-9-]*$ ]]`, which anchors to the entire slug (not per line) and rejects the embedded newline. **Re-verified (orchestrator + @qa):** the buggy grep gate accepts `decision-log`+newline+`x -->evil<!--`; the shipped `[[ =~ ]]` gate rejects it plus `good123`+newline+`rm -rf /`, while still accepting `decision-log`/`good123` and rejecting all single-line malicious cases. Downstream: the line-oriented bug would also have defeated the AC-SAFE-2 `grep -cF` idempotency re-detection on a multi-line marker; the whole-string slug gate closes that too. **Lesson (recorded):** a charset gate written with a line-oriented tool is not a whole-string gate — the check-that-cannot-fail pattern in a new guise, exposed only because the adversary built a *novel* payload class (embedded newline) rather than re-running the known fixtures.
