...
4. **Forbidden-token re-scan, independent of any earlier scan.** Re-run the CONTRIBUTING.md-recipe scan against the file at promotion time, regardless of whether it passed this same scan at generation time:

   ```bash
   grep -inE '\b(Ignore|Disregard|Override|Instead of|Always respond|New instruction)\b' ".claude/skills/<slug>/SKILL.md"
   ```

   Any match outside a fenced code block or an `<!-- HTML comment -->` refuses promotion. This is the backstop against a file that was clean when generated but altered afterward, and against a file that was never generated by `skill-studio` at all — a hand-copied file gets no exemption from this scan.
...
**A maintainer promoting from a checkout with write access is gated by review discipline, not by a technical barrier that existed before this release.** As of this same release, that convention is reinforced by two pieces landing together, and it matters what each one actually does: branch protection on `main` (require a PR, block direct pushes and force-pushes), enabled by the maintainer immediately after this release merges, **does** structurally close the "push straight to `main`" path — a promotion can no longer land without going through a PR at all, for anyone. A [CODEOWNERS](.github/CODEOWNERS) entry covering `skills/` and `curated-skills-registry.md` is added in this same release too, and auto-requests the named owner as a reviewer on any promotion PR — but branch protection is configured with zero required approvals (the practical setting for a single maintainer, so the maintainer isn't locked out of merging their own repo), so CODEOWNERS here is a visibility aid, not an approval gate that blocks a merge without a second sign-off. Until branch protection is turned on, a maintainer could still technically push straight to `main`, bypassing this ceremony entirely. This document does not claim a stronger gate exists for the maintainer than what's actually configured.
